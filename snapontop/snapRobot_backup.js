    ...
    if (situation !== 'refreshPalette') {
        ...
        if (this.isAppMode) {
        	  if (!this.stage.enableFullScreen){
        			...
        			this.stage.backgrounddimensions = new Point(this.width(),this.height()-this.logo.height());
        			StageMorph.prototype.backgrounddimensions = this.stage.backgrounddimensions;
            	this.stage.resetExtent(new Point(this.width(),this.height()-this.logo.height()));
    ...
    return this.scripts.children.filter(function (morph) {
        if (morph.selector) {
            ...
            if (morph.selector === 'receiveRemoteEvent') {
                return morph.inputs()[0].evaluate()[0] === interaction || morph.inputs()[0].contents().text === interaction;
            }            
        }
        return false;
    });
